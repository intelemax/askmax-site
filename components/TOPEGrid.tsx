// components/TOPEGrid.tsx

import React from "react";
import { TOPECard, TOPEPersona } from "./TOPECard";

const TOPE_PERSONAS: TOPEPersona[] = [
...
export const TOPEGrid: React.FC = () => {
  return (
    <section className="py-20 px-6 bg-white text-black text-center">
      <h2 className="text-3xl md:text-4xl font-bold mb-10">
        Meet the Persona Experts
      </h2>
      <p className="text-lg text-gray-600 mb-12 max-w-3xl mx-auto">
        Max is filtered through 29 symbolic perspectives—real domains, real stakes.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {TOPE_PERSONAS.map((persona, idx) => (
          <TOPECard key={idx} persona={persona} />
        ))}
      </div>
    </section>
  );
